        ...
        <section className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {blogs.slice(1).map((blog) => (
            <Link
              key={blog.id}
              to={`/blogs/${blog.id}`}
              className="group block overflow-hidden rounded-[30px] border border-[#e1cfab] bg-white shadow-[0_12px_45px_rgba(40,24,13,0.04)] transition-all hover:-translate-y-1 hover:shadow-[0_18px_55px_rgba(40,24,13,0.08)]"
            >
              ...
              <div className="p-6 space-y-3">
                ...
                <h3 className="font-display text-2xl text-[#1a120e] group-hover:text-[#9e6c24] transition-colors line-clamp-2 leading-tight">
                  {blog.title}
                </h3>
                <p className="text-stone-600 text-sm leading-relaxed line-clamp-3">
                  {blog.excerpt}
                </p>
                <div className="pt-2 flex items-center gap-1.5 text-xs font-bold text-[#1a120e] uppercase tracking-wider group-hover:underline">
                  Read More <BookOpen className="h-3.5 w-3.5" />
                </div>
              </div>
            </Link>
          ))}
