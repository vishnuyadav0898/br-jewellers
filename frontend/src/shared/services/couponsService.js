  ...
  async getAssignedCoupons(userId = "") {
    try {
      ...
      return list.map((item) => ({
        _id: item._id,
        coupon: item.coupon ? normalizeApiCoupon(item.coupon) : null,
        user: item.user ? { _id: item.user._id, name: item.user.name, email: item.user.email } : null,
